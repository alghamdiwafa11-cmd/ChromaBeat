...
const SubscriptionModal: React.FC<SubscriptionModalProps> = ({ isOpen, onClose, onSuccess }) => {
        ...
        (window as any).paypal.Buttons({
          ...
          onApprove: async (data: any, actions: any) => {
            ...
            try {
              ...
              setError("Subscription failed. Check connection.");
              setIsProcessing(false);
            }
          },
          onError: (err: any) => {
            setError("PayPal connection error.");
          },
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative glass w-full max-w-xl rounded-[3.5rem] overflow-hidden border-white/10 shadow-[0_0_150px_rgba(59,130,246,0.1)] animate-in fade-in zoom-in duration-500 max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-10 md:p-16">
          ...
          <div className="px-4 mb-8">
            <label className="flex items-start gap-4 cursor-pointer group">
              ...
              <span className="text-[10px] text-white/30 leading-relaxed font-bold uppercase tracking-wider group-hover:text-white/50 transition-colors">
                I authorize a monthly charge of £10 for Pro status. Cancel anytime via settings.
              </span>
            </label>
            {error && <div className="mt-4 flex items-center gap-2 text-red-500 text-[10px] font-black uppercase"><AlertCircle className="w-4 h-4" /> {error}</div>}
          </div>
          ...
          <div className="space-y-4">
            {!agreed ? (
              <div className="w-full bg-white/5 py-10 rounded-3xl flex flex-col items-center justify-center border border-white/5 border-dashed">
                 <p className="text-[10px] font-black text-white/20 uppercase tracking-[0.4em]">Review Monthly Terms Above</p>
              </div>
            ) : (
              <div className="relative min-h-[150px]">
                {isProcessing && (
                  <div className="absolute inset-0 bg-black/50 z-10 flex flex-col items-center justify-center rounded-3xl backdrop-blur-sm">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest">Linking Secure Account...</p>
                  </div>
                )}
          ...
          <div className="mt-10 flex items-center justify-center gap-8 border-t border-white/5 pt-10">
            <span className="flex items-center gap-2 text-[10px] text-white/10 font-black uppercase tracking-[0.2em]"><ShieldCheck className="w-4 h-4" /> Secure Auth</span>
            <span className="flex items-center gap-2 text-[10px] text-white/10 font-black uppercase tracking-[0.2em]"><Lock className="w-4 h-4" /> PayPal </span>
          </div>
        </div>
      </div>
    </div>
