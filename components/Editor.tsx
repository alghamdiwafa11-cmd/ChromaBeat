...
const Editor: React.FC<EditorProps> = ({ appState, onBack, onExported, onUpdateBackground, onRequestUpgrade }) => {
  ...
  const [settings, setSettings] = useState<VisualizerSettings>({
    ...
    sensitivity: 50, intensity: 100, placementX: 50, placementY: 50, barWidth: 1.2, filter: '',
  ...
  const handleVeoGen = async () => {
    ...
    try {
      const url = await gemini.generateBackgroundVideo(appState.metadata?.imagePrompt || "Abstract rhythm", setVeoProgress);
  ...
  const handleAIVisualGen = async () => {
    ...
    try {
      const url = await gemini.generateBackgroundImage(appState.metadata?.imagePrompt || "Cinematic atmosphere");
  ...
  return (
    <div className="flex h-screen bg-[#050505] overflow-hidden text-white font-sans">
      <aside className="w-80 glass border-r border-white/10 overflow-y-auto p-6 space-y-8 z-20 custom-scrollbar">
        <button onClick={onBack} className="flex items-center gap-2 text-white/40 hover:text-white transition group mb-2 font-black uppercase tracking-widest text-[10px]">
          <ChevronLeft className="w-4 h-4" /> Exit Editor
        </button>
        
        <section className="space-y-4">
          <h3 className="text-[10px] font-black uppercase tracking-widest text-white/40 flex items-center gap-2"><Palette className="w-3 h-3 text-cyan-400" /> Styles & Vibe</h3>
          <div className="grid grid-cols-3 gap-2">
            {COLOR_PALETTES.map(p => (
              <button key={p.name} onClick={() => setSettings({...settings, color: p.primary, colorSecondary: p.secondary})} className="h-8 rounded-lg transition hover:scale-105 active:scale-95" style={{ background: `linear-gradient(45deg, ${p.primary}, ${p.secondary})` }} title={p.name} />
            ))}
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="flex items-center gap-4">
            <span className="text-[9px] font-black text-white/20 uppercase">Size</span>
            <input type="range" min="24" max="82" value={settings.lyricsSize} onChange={(e) => setSettings({...settings, lyricsSize: parseInt(e.target.value)})} className="flex-1 accent-cyan-500" />
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <button onClick={() => setSettings({...settings, aspectRatio: '9:16'})} className={`p-2 rounded-xl border flex flex-col items-center gap-1 transition ${settings.aspectRatio === '9:16' ? 'bg-cyan-500/20 border-cyan-500 text-cyan-400' : 'bg-white/5 border-white/10 text-white/40'}`}>
              <Smartphone className="w-4 h-4" /><span className="text-[8px] font-black">MOBILE</span>
            </button>
        ...
        <section className="space-y-4">
          <h3 className="text-[10px] font-black uppercase tracking-widest text-white/40 flex items-center gap-2"><Sparkles className="w-3 h-3 text-cyan-400" /> AI Visual Engine</h3>
        ...
        <div className="flex-1 relative overflow-hidden flex items-center justify-center p-12" ref={canvasContainerRef}>
          <div className={`relative shadow-[0_0_120px_rgba(0,0,0,0.8)] rounded-[3rem] overflow-hidden border border-white/5 bg-[#080808] transition-all duration-700 ${settings.aspectRatio === '9:16' ? 'h-full aspect-[9/16]' : settings.aspectRatio === '1:1' ? 'h-full aspect-square' : 'w-full aspect-video'}`}>
            ...
            <div className="absolute top-12 left-12 glass px-6 py-4 rounded-[2rem] flex items-center gap-4">
              ...
              <div><p className="text-xl font-black leading-tight tracking-tighter">{appState.metadata?.title}</p><p className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em]">{appState.metadata?.artist}</p></div>
            </div>
          </div>
        </div>
